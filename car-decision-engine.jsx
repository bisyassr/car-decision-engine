...
function computeTCO(v, profile, regional) {
  const yrs = profile.ownershipYears, km = profile.annualKm;
...
function costScore(v, all, profile, regional) {
  ...
  const mx = Math.max(...tcos), mn = Math.min(...tcos);
...
function riskScore(v) {
  const raw = 0.5 * (100 - v.reliabilityIndex) * 0.5 + 0.3 * (100 - v.recallScore) * 0.8 + 0.2 * (100 - v.techMaturity);
  return Math.max(0, 100 - raw);
}

function resaleScore(v) { return 0.7 * (1 - v.depreciationRate) * 100 + 0.3 * v.marketDemand; }
...
function scoreVehicle(v, p, all, regional) {
  ...
  return { vehicle: v, finalScore: Math.round(final * 10) / 10, costScore: Math.round(cs), reliabilityScore: Math.round(rs), fitScore: Math.round(fs), riskScore: Math.round(rks), resaleScore: Math.round(res), weights: w, tco: computeTCO(v, p, regional) };
}

function runEngine(p) {
  const r = REGIONAL_DATA[p.province] || REGIONAL_DATA.OTHER;
  return D.map(v => scoreVehicle(v, p, D, r)).filter(Boolean).sort((a, b) => b.finalScore - a.finalScore);
}
...
function evHybridCandidates(budget, bodyPref) {
  ...
  return {
    evs:     D.filter(v => v.fuelType === "ev"     && ok(v)).sort((a, b) => rank(b) - rank(a)).slice(0, 4),
    hybrids: D.filter(v => v.fuelType === "hybrid" && ok(v)).sort((a, b) => rank(b) - rank(a)).slice(0, 4),
  };
}

function buildBreakeven(ev, hy, reg, km) {
  const fp = reg.fuelPrice, inc = reg.evIncentive;
...
async function genExplanations(top3, profile) {
  const sums = top3.map((r, i) => `#${i+1}: ${r.vehicle.year} ${r.vehicle.make} ${r.vehicle.model} | Score:${r.finalScore} | $${r.vehicle.msrp.toLocaleString()} | TCO:${fmt(r.tco.tco)} | ${r.vehicle.fuelType}`).join("\n");
  const txt = await callClaude(`Expert Canadian car advisor. Write 2-sentence explanation for each of these 3 scored vehicles. Mention real numbers. Return ONLY JSON array: [{"rank":1,"headline":"...","explanation":"..."},{"rank":2,...},{"rank":3,...}]\n\nUser: $${profile.budget.toLocaleString()} budget | ${profile.province} | ${profile.annualKm.toLocaleString()}km/yr | ${profile.drivingType} | ${profile.ownershipYears}yr\n\nVehicles:\n${sums}`);
  try { return JSON.parse(txt.replace(/```json|```/g, "").trim()); }
  catch { return top3.map((r, i) => ({ rank: i+1, headline: "Strong match", explanation: `Scores ${r.finalScore}/100 with ${fmt(r.tco.tco)} 5-year TCO.` })); }
}
...
function Field({ label, hint, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#64748b", fontFamily: "monospace", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 7 }}>{label}</label>
      {children}
...
function VehicleCard({ result: r, rank, explanation: ex, isTop }) {
  ...
  return (
    <div style={{ background: isTop ? "linear-gradient(135deg,#0f172a,#1a1f35)" : "#0f172a", border:`1px solid ${isTop?"rgba(245,158,11,0.3)":"#1e293b"}`, borderRadius:16, padding:24, marginBottom:16, position:"relative", overflow:"hidden", cursor:"pointer", transition:"transform 0.2s" }}
      onClick={() => setOpen(o=>!o)} onMouseEnter={e=>e.currentTarget.style.transform="translateY(-2px)"} onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12 }}>
        <div style={{ flex:1 }}>
          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap" }}>
            <span style={{ fontSize:10,fontWeight:800,color:rc,background:`${rc}20`,padding:"3px 8px",borderRadius:4,fontFamily:"monospace",letterSpacing:"0.1em" }}>#{rank} {rl}</span>
            <span style={{ fontSize:10,fontWeight:700,color:"#fff",background:fc[r.vehicle.fuelType],padding:"3px 8px",borderRadius:4,fontFamily:"monospace",textTransform:"uppercase" }}>{r.vehicle.fuelType}</span>
          </div>
          <h3 style={{ margin:0,fontSize:20,fontWeight:800,color:"#f1f5f9",fontFamily:"Georgia,serif",letterSpacing:"-0.02em" }}>{r.vehicle.year} {r.vehicle.make} {r.vehicle.model}</h3>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginTop:16 }}>
        {[["5-Year TCO",fmt(r.tco.tco)],["Fuel / 5yr",fmt(r.tco.fuelCost)],["Resale",fmt(r.tco.resaleValue)]].map(([l,v])=>(
      ...
      {open && (
        <div style={{ marginTop:16,padding:16,background:"#0d1117",borderRadius:8 }}>
          ...
          <div style={{ marginTop:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
            {[["Insurance 5yr",fmt(r.tco.insuranceCost)],["Maintenance 5yr",fmt(r.tco.maintenanceCost)],["AWD",r.vehicle.awd?"✓ Yes":"✗ No"],["Safety",`${r.vehicle.safetyRating}/5 ★`]].map(([l,v])=>(
...
function CostChart({ yrs, evC, hyC, bkYr }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%",height:"auto" }}>
      {[0,0.33,0.66,1].map(f=>{
        const y=pT+iH*(1-f),val=mn+rng*f;
        return <g key={f}><line x1={pL} y1={y} x2={W-pR} y2={y} stroke="#1e293b" strokeWidth="1"/><text x={pL-4} y={y+4} textAnchor="end" fontSize="8" fill="#475569" fontFamily="monospace">${Math.round(val/1000)}k</text></g>;
      })}
      {yrs.map(y=><text key={y} x={xS(y)} y={H-3} textAnchor="middle" fontSize="8" fill="#475569" fontFamily="monospace">Y{y}</text>)}
      {bkX&&<><line x1={bkX} y1={pT} x2={bkX} y2={pT+iH} stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="4,3"/><text x={bkX+3} y={pT+11} fontSize="8" fill="#f59e0b" fontFamily="monospace">Breakeven</text></>}
    ...
    <div>
      <div style={{ marginBottom:22 }}>
        <div style={{ display:"flex",gap:8,alignItems:"center",marginBottom:6 }}>
          ...
          <span style={{ fontSize:10,color:"#334155",fontFamily:"monospace" }}>{profile.province} · {profile.annualKm.toLocaleString()} km/yr</span>
        </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18 }}>
        <div style={{ background:"#0d1117",border:"1px solid #3b82f630",borderRadius:10,padding:14 }}>
          ...
          <p style={{ margin:"7px 0 0",fontSize:11,color:"#475569",fontFamily:"monospace" }}>After ${reg.evIncentive.toLocaleString()} incentive: <span style={{ color:"#3b82f6",fontWeight:700 }}>${(ev.msrp-reg.evIncentive).toLocaleString()}</span></p>
        </div>
        <div style={{ background:"#0d1117",border:"1px solid #10b98130",borderRadius:10,padding:14 }}>
          ...
          <p style={{ margin:"7px 0 0",fontSize:11,color:"#475569",fontFamily:"monospace" }}>Fuel economy: <span style={{ color:"#10b981",fontWeight:700 }}>{hy.fuelEcon} L/100km</span></p>
        </div>
      </div>

      {/* TCO Banner */}
      <div style={{ background:evW?"linear-gradient(135deg,#0d1a2e,#0f172a)":"linear-gradient(135deg,#0a1f16,#0f172a)",border:`1px solid ${evW?"#3b82f640":"#10b98140"}`,borderRadius:12,padding:"16px 20px",marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12 }}>
        <div>
          <p style={{ margin:"0 0 4px",fontSize:10,color:"#475569",fontFamily:"monospace",textTransform:"uppercase" }}>5-Year TCO Winner</p>
          <p style={{ margin:"0 0 4px",fontSize:18,fontWeight:900,color:evW?"#3b82f6":"#10b981",fontFamily:"monospace" }}>{evW?`${ev.make} ${ev.model} (EV)`:`${hy.make} ${hy.model} (Hybrid)`}</p>
          <p style={{ margin:0,fontSize:12,color:"#64748b",fontFamily:"monospace" }}>Saves <span style={{ color:"#f59e0b",fontWeight:700 }}>${Math.round(Math.abs(sc.evT-sc.hyT)).toLocaleString()}</span> over 5 years</p>
        </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:18 }}>
        {[
          { label:"Breakeven",  ev:bk.bkYr?`Yr ${bk.bkYr}`:">10 yrs", hy:"Day 1", ec:bk.bkYr&&bk.bkYr<=profile.ownershipYears?"#3b82f6":"#ef4444", note:bk.bkYr&&bk.bkYr<=profile.ownershipYears?"Within ownership":"Doesn't break even" },
      ...
      <div style={{ background:"#0d1117",border:"1px solid #1e293b",borderRadius:12,padding:16,marginBottom:18 }}>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",marginTop:12,paddingTop:12,borderTop:"1px solid #1e293b" }}>
          <span style={{ fontSize:12,color:"#3b82f6",fontFamily:"monospace",fontWeight:700 }}>EV wins: {sc.dims.filter(d=>d.ev>=d.hybrid).length}/{sc.dims.length}</span>
          <span style={{ fontSize:12,color:"#10b981",fontFamily:"monospace",fontWeight:700 }}>Hybrid wins: {sc.dims.filter(d=>d.hybrid>d.ev).length}/{sc.dims.length}</span>
        </div>
      </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18 }}>
        {[
          { label:"EV Annual Costs", color:"#3b82f6", icon:"⚡", rows:[["Energy (elec)",Math.round(sc.ef5/5)],["Insurance/yr",Math.round(ev.insuranceBaseYear*reg.insuranceMultiplier)],["Maintenance/yr",ev.maintenanceCostYear],["Total/yr",Math.round(sc.ef5/5)+Math.round(ev.insuranceBaseYear*reg.insuranceMultiplier)+ev.maintenanceCostYear]] },
          { label:"Hybrid Annual Costs", color:"#10b981", icon:"🌿", rows:[["Fuel (gas)",Math.round(sc.hf5/5)],["Insurance/yr",Math.round(hy.insuranceBaseYear*reg.insuranceMultiplier)],["Maintenance/yr",hy.maintenanceCostYear],["Total/yr",Math.round(sc.hf5/5)+Math.round(hy.insuranceBaseYear*reg.insuranceMultiplier)+hy.maintenanceCostYear]] },
        ].map((col,ci)=>(
          <div key={ci} style={{ background:"#0d1117",border:`1px solid ${col.color}25`,borderRadius:10,padding:14 }}>
            <p style={{ margin:"0 0 10px",fontSize:10,color:col.color,fontFamily:"monospace",textTransform:"uppercase" }}>{col.icon} {col.label}</p>
            {col.rows.map(([l,v],ri)=>(
      ...
      {profile.coldClimate && (
        <div style={{ background:"#1a1200",border:"1px solid #f59e0b30",borderRadius:10,padding:"12px 16px",marginBottom:18 }}>
          ...
          <p style={{ margin:0,fontSize:12,color:"#94a3b8",lineHeight:1.6 }}>EV range drops 20–40% in cold Canadian winters. Home charging costs $800–$2,000. Hybrids have no range penalty and provide instant cabin heat.</p>
        </div>
      )}

      {/* AI Verdict */}
      <button onClick={handleVerdict} disabled={loading} style={{ width:"100%",padding:"14px",background:loading?"#1e293b":"linear-gradient(135deg,#1d4ed8,#2563eb)",border:"none",borderRadius:10,fontSize:13,fontWeight:800,color:loading?"#475569":"#fff",fontFamily:"monospace",textTransform:"uppercase",letterSpacing:"0.1em",cursor:loading?"not-allowed":"pointer",marginBottom:verdict?16:0,boxShadow:loading?"none":"0 4px 20px rgba(37,99,235,0.3)" }}>
      ...
      {verdict && (
        <div style={{ background:"linear-gradient(135deg,#0f172a,#1a1f35)",border:"1px solid #1d4ed840",borderRadius:12,padding:20 }}>
          <p style={{ margin:"0 0 12px",fontSize:10,color:"#3b82f6",fontFamily:"monospace",textTransform:"uppercase",letterSpacing:"0.12em",fontWeight:700 }}>⚡ AI Verdict · {ev.make} {ev.model} vs {hy.make} {hy.model}</p>
...
export default function CarDecisionEngine() {
  ...
  const handleAnalyze = useCallback(async () => {
    setLoading(true); setTab("results"); setExplanations(null);
    try {
      ...
      <div style={{ background:"linear-gradient(180deg,#0d1117,#070b14)",borderBottom:"1px solid #1e293b",padding:"24px 24px 0" }}>
        <div style={{ maxWidth:760,margin:"0 auto" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:4 }}>
            ...
            <span style={{ fontSize:10,fontFamily:"monospace",color:"#475569",textTransform:"uppercase",letterSpacing:"0.15em" }}>v2.0 · 45 Vehicles · Deterministic Engine</span>
          </div>
          ...
          <div style={{ display:"flex",gap:0,borderBottom:"1px solid #1e293b" }}>
            {[["form","Profile Input"],["results","Recommendations"],["ev-hybrid","⚡ EV vs Hybrid"]].map(([k,l])=>(
              <button key={k} onClick={()=>setTab(k)} style={{ background:"none",border:"none",cursor:"pointer",padding:"10px 18px",fontSize:11,fontFamily:"monospace",textTransform:"uppercase",letterSpacing:"0.08em",color:tab===k?"#f59e0b":"#475569",borderBottom:tab===k?"2px solid #f59e0b":"2px solid transparent",marginBottom:-1,transition:"all 0.2s",whiteSpace:"nowrap" }}>{l}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:760,margin:"0 auto",padding:"28px 24px" }}>
        ...
        {tab==="form" && (
          <div>
            ...
            <Field label={`Priority: ${profile.priority==="balanced"?"Balanced":profile.priority==="cost"?"Cost First":profile.priority==="reliability"?"Reliability First":"Comfort First"}`} hint="Adjusts scoring weights dynamically">
            ...
            <div style={{ background:"#0d1117",border:"1px solid #1e293b",borderRadius:10,padding:"14px 16px",marginBottom:24 }}>
              ...
              <div style={{ display:"flex",gap:24,flexWrap:"wrap" }}>
                {[["Fuel Price",`$${reg.fuelPrice}/L`],["Insurance Adj.",`×${reg.insuranceMultiplier}`],["EV Incentive",`−$${reg.evIncentive.toLocaleString()}`],["Electricity",`$${ELECTRICITY_PRICE}/kWh`]].map(([l,v])=>(
                  <div key={l}><div style={{ fontSize:10,color:"#334155",fontFamily:"monospace" }}>{l}</div><div style={{ fontSize:13,color:"#f59e0b",fontFamily:"monospace",fontWeight:700 }}>{v}</div></div>
                ))}
              </div>
            </div>
            <button onClick={handleAnalyze} style={{ width:"100%",padding:"16px",background:"linear-gradient(135deg,#f59e0b,#d97706)",border:"none",borderRadius:10,fontSize:15,fontWeight:800,color:"#0d1117",fontFamily:"monospace",textTransform:"uppercase",letterSpacing:"0.1em",cursor:"pointer",boxShadow:"0 4px 20px rgba(245,158,11,0.3)" }}
          ...
          <div>
            ...
            {!results && !loading && (
              <div style={{ textAlign:"center",padding:"60px 0",color:"#475569",fontFamily:"monospace" }}>
                ...
                <button onClick={()=>setTab("form")} style={{ marginTop:12,padding:"10px 20px",background:"#1e293b",border:"none",borderRadius:8,color:"#94a3b8",fontFamily:"monospace",cursor:"pointer" }}>← Profile Input</button>
              </div>
            )}
            {results && (
              <>
                ...
                {results.top3.map((r,i)=><VehicleCard key={r.vehicle.id} result={r} rank={i+1} explanation={explanations?.[i]} isTop={i===0}/>)}
                    ...
                    <div style={{ background:"#0d1117",border:"1px solid #1e293b",borderRadius:10,overflow:"hidden" }}>
                      {results.all.slice(3,10).map((r,i)=>(
                        <div key={r.vehicle.id} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 16px",borderBottom:i<6?"1px solid #1e293b":"none" }}>
                          <span style={{ fontSize:13,color:"#94a3b8" }}>#{i+4} {r.vehicle.year} {r.vehicle.make} {r.vehicle.model}</span>
                ...
                <button onClick={()=>setTab("form")} style={{ marginTop:24,width:"100%",padding:"13px",background:"none",border:"1px solid #1e293b",borderRadius:10,fontSize:13,fontFamily:"monospace",color:"#475569",cursor:"pointer",textTransform:"uppercase" }}>← Adjust Profile</button>
              </>
            )}
          </div>
        )}
